...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($teacher_id) && !empty($subject_id) && !empty($academic_year) && !empty($semester)) {
        $check_query = "SELECT assignment_id FROM teacher_subjects WHERE teacher_id = ? AND subject_id = ? AND academic_year = ? AND semester = ?";
        $check_stmt = $conn->prepare($check_query);
        $check_stmt->bind_param("iiss", $teacher_id, $subject_id, $academic_year, $semester);
        $check_stmt->execute();
    ...
    if (empty($errors)) {
        $insert_query = "INSERT INTO teacher_subjects (teacher_id, subject_id, academic_year, semester) VALUES (?, ?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_query);
        $insert_stmt->bind_param("iiss", $teacher_id, $subject_id, $academic_year, $semester);
        
        if ($insert_stmt->execute()) {
            redirect('/school/assignments/index.php', 'Assignment added successfully', 'success');
        } else {
            $errors[] = "Error adding assignment: " . $conn->error;
        }
    }
}
...
<div class="row">
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2>Add New Teacher-Subject Assignment</h2>
            <a href="/school/assignments/index.php" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Back to Assignments</a>
        </div>
            ...
            <form method="POST" action="">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="subject_id" name="subject_id" required>
                            ...
                            <?php while ($subject = $subjects_result->fetch_assoc()): ?>
                                <option value="<?php echo $subject['subject_id']; ?>" <?php echo (isset($_POST['subject_id']) && $_POST['subject_id'] == $subject['subject_id']) ? 'selected' : ''; ?>>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="academic_year" name="academic_year" value="<?php echo isset($_POST['academic_year']) ? htmlspecialchars($_POST['academic_year']) : $default_academic_year; ?>" placeholder="e.g. 2023-2024" required>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select class="form-select" id="semester" name="semester" required>
                            ...
                            <option value="Winter" <?php echo (isset($_POST['semester']) && $_POST['semester'] == 'Winter') ? 'selected' : ''; ?>>Winter</option>
                        </select>
                    </div>
                </div>
